...
% dx(t)=g*f(x)dB(t)+s8gma*Sigma(x)dL(t)

%%% Stochastic Logistic model with State dependet Carrying Capacity K
% function [X,U]=tgao7_Dwell(Alp,T)
tic;
clear
alpha=1.0;    T=50;          %%%%change
  
 eps=0.3;
% cigma=@(x)  abs(x).^alpha.*sign(x);
eps=eps^alpha;
cigma=@(x)  eps*abs(x).^alpha;
d=0;
x0=0.5; gr=0.8; % growth rate
S=1;
k1=0.8; k2=1.2; s=k2-k1;   phi=(k1+k2)/2; gam=8;% gamma must be near gamC
V=@(x)k1+(k2-k1)/(1+exp(-gam*(x-phi)));%  State dependet Carrying Capacity K
gamC=4/(k2-k1) % bifurcation poin 
f=@(x) gr*x.*(1-x./(k1+s./(1+exp(-gam*(x-phi)))));% LOGSTIC Growth model with state-dependent C.Capacity
%f=@(x) gr*x.*(x./S-1).*(1-x./(k1+s./(1+exp(-gam*(x-phi)))));% ALLEE EFFECT model
rr=1;             %right
rl=1;             %left
fpmax=abs(f(-rr));
...
%legend('\sigma=0.1','\sigma=0.4','\sigma=0.7','\sigma=1')
 %legend('s=0.01', 's=0.05', 's=0.10','s=0.20')
 ylabel('pdf-p')
 hold on
 %grid on
